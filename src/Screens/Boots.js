...
import img1 from '../Assets/whiteb.png';
import { ChelseaBoots } from '../FileCollection/ChelseaBoots';
			...
			<DivWrapper>
				<GridContainer container>
					<GridItem item xs={8} sm={6} md={4} lg={3} style={{ display: 'block' }}>
						<H2>Chelsea {selectedText} Boot</H2>
						<H3>Price: Rs. 1200</H3>
						<PBdiv>
					...
					<GridItem item xs={8} sm={6} md={4} lg={4} style={{ display: 'block' }}>
						...
						>
							{ChelseaBoots.map((boot, index) => {
								return (
									<StyledButtons
										active={active === boot.color ? true : false}
