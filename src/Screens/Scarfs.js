...
import { ChelseaScarfs } from '../FileCollection/ChelseaScarfs';
			...
			<DivWrapper>
				<GridContainer container>
					<GridItem item xs={8} sm={6} md={4} lg={3} style={{ display: 'block' }}>
						<H2>Chelsea {selectedText} Scarf</H2>
						<H3>Price: Rs. 1000</H3>
						<PBdiv>
					...
					<GridItem item xs={8} sm={6} md={4} lg={4} style={{ display: 'block' }}>
						...
						>
							{ChelseaScarfs.map((scarf, index) => {
								return (
									<StyledButtons
										active={active === scarf.color ? true : false}
